...
function Clock({
  ...
  }, [isRunning, resetSignal, escala, tipoFuncao, finalizado, onFinish])
  ...
  const svgSize = isMobile ? 80 : 120
  const center = svgSize / 2
  const mobileRadius = isMobile ? 30 : radius
  const strokeWidth = isMobile ? 6 : 10

  return (
    <div className="clock-panel">
      <h3 className="clock-title">{title}</h3>
      <div className="clock-bigoh">
        Complexidade: <InlineMath math={bigOh} />
      </div>
      <div className="clock-circle">
        <svg width={svgSize} height={svgSize}>
          ...
          <circle
            ...
            strokeDashoffset={
              2 * Math.PI * mobileRadius * (1 - percentual / 100)
            }
          />
        </svg>
      ...
      <p className="estimated-time">
        Tempo estimado:{" "}
        {tempoTotalRef.current === 0
          ? "Instantâneo"
          : tempoTotalRef.current >= 1000 * 3600 * 24 * 365
          ? Math.round(tempoTotalRef.current / (1000 * 3600 * 24 * 365)) +
            " anos"

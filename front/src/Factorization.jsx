...
function Factorization() {
  ...
  const fatorar = async (e) => {
    ...
    setResultado('');
    try {
      const res = await fetch(`${API_URL}/factorize?number=${numero}&type_alg=${algoritmo}`);
    ...
    <div className="card">
      ...
      <form onSubmit={fatorar}>
        <div className="mb-3 text-start">
          ...
          <input type="number" className="form-control" value={numero} onChange={e => setNumero(e.target.value)} placeholder="Digite um número"/>
        </div>
        <div className="mb-3 text-start">
          <label className="form-label">Algoritmo:</label>
          <select className="form-select" value={algoritmo} onChange={e => setAlgoritmo(e.target.value)}>
